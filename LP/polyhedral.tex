...
Finally, observe from the proof of Theorem~\ref{thm: FM} that, if $\bmb = \bm0$ initially, then the new bounds produced by the algorithm remains $\{b_1', b_2', \dots, b_{m'}'\} = \{0\}$. 
...
Geometrically, Theorem~\ref{thm: extreme points part 1} characterizes an extreme point as the unique solution that attains $m$ linearly independent constraint hyperplanes in $\rr^m$.
...
Finally, we may prove Theorem~\ref{thm: extreme points part 3}, which is essentially the bounded version of Theorem~\ref{thm: extreme points part 2}. Though the proofs are parallel, the techniques are identical. Let $\bmu, \bmw, \bmv\in\rr^m,$ such that $\bmu \leq \bmw$, and $u_i, w_i, v_i$ be the $i$-th entry; we denote $\supp_{\bmu, \bmw}(\bmv) = \{i: u_i < v_i < w_i\}$. The proof of Theorem~\ref{thm: extreme points part 3} is identical to the proof for Theorem~\ref{thm: extreme points part 2} with some minor twists in the setup. \\ 
...
\begin{proof}[Sketch of Proof for Theorem~\ref{thm: extreme points part 3}]
...
We examine and emphasize the existence of extreme points in a finitely generated polyhedral set. Recall the hypothesis in Theorem~\ref{thm: extreme points part 2} and~\ref{thm: extreme points part 3} that we are given a polyhedral set with only equalities. Consider a linear program (\ref{P}) that optimizes over a polyhedral set $M\bmx\leq b$; the program is in {\em standard form} if all the inequalities are equalities; similarly, if we consider a program that only contains equality constraints, we also call the system {\em in standard form}. In a later chapter, we will see why a program in standard form is necessary and sufficient for constructing a solution. With respect to a point $\bmx\rast\in\rr^{n}$, a row (a constraint) $\bma_i^\top$ is said to be {\em active} if $\inner{\bma, \bmx\rast} = b_i$. 



\begin{definition}
Let $P$ be a polyhedral set in $\rr^m$ defined by a list of constraints and bounds $\bme_1, \bme_2, \dots, \bme_r, \bma_1, \bma_2, \dots, \bma_n \in\rr^{m}$ and $e_1, e_2, \dots, e_r, b_1, b_2, \dots, b_n \in \rr$ so that 
\[\begin{array}{rl}
P = \{\bmx :&\forall i\in\{1,2,\dots, r\}, \inner{\bme_i, \bmx} = e_i,  \\ 
&\forall j\in\{1,2,\dots, n\},\inner{\bma_j, \bmx} \leq b_j~\}.
\end{array}\] Let $\bmx\rast\in \rr^{n}$.  
\begin{itemize}
\item 
...
We should remind reader junior readers that the term ``basic'' stands for the adjective for ``basis'', which states that the solution is generated by a chosen linear algebraic basis. In the following, we consider a linear algebraic property of the intersection of a convex set and a hyperplane and the behavior of an extreme point under intersections. 

%Though we will omit the formal definition of the dimension of a subspace, the following lemma will be helpful. 
...
\end{corollary}
\begin{proof}
Let $P = \conv(X) + C$ be a polyhedral set with its Minkowski-Weyl representation. A polyhedral set is closed and convex; if $P$ is compact, then $P$ is bounded, which means $C = \{\bm0\}$. Toward a contradiction, suppose $\bmx \in X$ is not an extreme point of $P$, then $\bmx = \alpha \bmu +  (1-\alpha)\bmw$ for some $\alpha\in(0,1)$ and $\bmu, \bmw \in P$, where the representation of $\bmu$ and $\bmw$ can be $\bmx$-free (i.e. the coefficient of $\bmx$ in the representation is 0). Thus, $\conv(X) = \conv(X\setminus\{\bmx\})$. The statement follows by inductively removing all the non-extreme points.
\end{proof}
