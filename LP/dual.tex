...
Let $M\in\rr^{m\times n}, \bmb\in\rr^m, \bmc\in\rr^n$; consider the standard primal linear program in inequlity form: 
\begin{equation*}
\begin{split}
...
where $m_{i, j}$ is the $(i, j)$-th entry of $M$, $b_j$ and $x_j$ are the entry on the $i$-th coordinate of $\bmb$ and $\bmx$, respectively, for $i \in \{1, 2, \dots, m\}$ and $j \in \{1, 2, \dots, n\}$. 

Many textbooks further assume $\bmx\geq\bm0$ and, by a seemingly symmetrical rule, introduce a vector $\bmy\in \rr^m$ of dual variables to define the dual as 
\begin{equation*}
\begin{array}{rrl}
...
\end{array}
\end{equation*}
The connection between these two problems is often justified a posteriori by the following: for any feasible $\bmx$ and $\bmy$, i.e., $M\bm x\leq \bmb$ and $M^\top\bm y\geq \bmc$, we have
...
However, there is a logic gap: the system of inequalities $M^\top\bmy \geq \bmc$ in the dual problem is introduced because the chain of inequalities holds. While this verifies the duality relationship, it does not explain where the dual variables come from, why the signs are what they are, or how to generalize the construction to other constraints. 

In this section, we take a different pedagogical route. Instead of postulating the dual and proving it works, we derive the dual directly from the geometry of the primal system of linear inequalities; namely, Farkas' Lemma (Theorem~\ref{farkas}). We then prove a minimax theorem, Theorem~\ref{minimax}, which reveals that any method in convex optimization with Lagrangians can be cast into a pure feasibility problem via Farkas' Lemma, thereby showing that optimization and feasibility are two sides of the same geometric coin. Finally, we define a general linear optimization (\ref{GP}) and state the form of its dual (\ref{GP}) in order to derive it. 
...
\end{array}
\end{equation}
and we may view $M$ as linear transformation $M:\rr^n \to \rr^m$; in a way, the optimization problem (P0) is to study the row space of the matrix $M$, where one needs to find a unitary vector in $\rr^n$ so that it is closest to the objective $\bmc$ and obey the constraint $M$.  
...
Let $M\in\rr^{m\times n}, \bmb\in\rr^m$ and $x_1,$ $x_2$, $\dots$, $x_n$, $y_1$, $y_2$, $\dots$, $y_m$ be a list of variables written in their vector forms $\bmx\in\rr^n$ and $\bmu\in\rr^m$. Consider the following two systems of linear inequalities. 
...
Finally, we assume $\bms = 0$, we have that $ \inner{\bm u\rast, M\bmx} + \inner{\bmu\rast, \bm s} =  \inner{\bmu\rast, M\bmx} = \inner{M^\top \bmu\rast, \bmx} \geq 0$. Now, by properties of inner products, for every $\bmx\in\rr^n$, we have that $\inner{M^\top \bmu\rast, -x} = -\inner{M^\top \bmu\rast, \bmx}\leq 0$. Which then implies that $\inner{M^\top \bmu\rast, \bmx} = 0$ as it is the only number that equals its own negation. By the positive-semidefiniteness of inner products, it follows that $M^\top \bmu\rast = 0$. 
\end{proof}

Observe that, we want only the solution where $M\bmx\leq \bmb$ in (\ref{P0}), which is equivalent to $M\bmx - \bmb\in\rr^n_-$ and, otherwise, discard may as $-\infty$. Consider the following notion to reformulate the original problem (\ref{P0}) in a more analytical form.  
...
\begin{equation}\label{a}
 ...
 \end{equation}  
where $z\rast\in\rr\cup\{-\infty\}$.  Recall that, given a cone $S\subseteq V$, the polar cone is denoted and defined as $S^\circ = \{\bmy\in V: \forall \bmx\in V, \inner{\bmx, \bmy}\leq 0\}$. 
...
$\delta_{S}(x) = \sup_{u\in S^\circ}\{\inner{x, u}\}.$\hfill \qedsymbol
\end{proposition} 
...
\begin{align*}
\sup_{x\in X} L(\bmx, \bmu_0) = & \sup_{\bmx\in X}\{\inner{\bmc, \bmx} + \inner{\bmu_0, \bmb}-\inner{\bmu_0, M \bmx}\} \\
	= 	& \inner{\bmu_0, \bmb} + \sup_{\bmx\in \rr^n}\{ \inner{\bmc, \bmx}-\inner{\bmu, M \bmx}\} \\ 
	= 	& \inner{\bmu_0, \bmb} + \sup_{\bmx\in \rr^n}\{ \inner{\bmc - M^\top \bmu_0, \bmx}\} \\ 
	= 	& \inner{\bmu_0, \bmb} + \delta_{\{\bmu: \bmc- M^\top \bmu = 0\} }(\bmu_0). \\ 
 \end{align*}
By the contradiction hypothesis and the definition of an infimum, fix any $u_0\in \rr_+^m$, we have the following chain of inequalities that
...
and $ \inner{\bmu_0, \bmb} + \delta_{\bmu: M^\top \bmu = \bmc}(u_0) \in\{ \inner{\bmu_0, \bmb}, \infty\}. $ Thus, as we verified the two sides of the strict inequality and the density of the real numbers, we are ensured that there is an $\alpha\in\rr$ so that 
...
has a solution. Let $\bmu\rast$ and $u_c\rast$ be a solution for the constraints (\ref{inhom}), and reformulate so we have 
...
$$ \inner{\widetilde\bmu, \bmb}  < \alpha< \inf_{u} \sup_{x} L( x, u) \leq \inner{\widetilde\bmu, \bmb}.$$ Alternatively, we assume $u_c\rast = 0$ and plug in to inequalities (\ref{eqn: aux D}) and
...
\begin{itemize}
\item In (\ref{eqn: D}), the constraint $A^\top y = 0$ is determined by the fact that $x$ is taken from the entire space of $\rr^n$. If we instead restrict $x\in\rr^n_+$ and change the proof accordingly, then the dual constraints in (\ref{eqn: D}) change to $A^\top y \geq 0$. Indeed, in the proof, when the slack variable satisfies $s = 0$, then $\inner{y\rast, Ax} = \inner{A^\top y\rast, x}\geq 0$ for all $x\geq 0$ if and only if $A^\top y\geq 0$. More generally, for any closed convex cone $C$, if $x\in C$, then the dual condition becomes 
\[ M^\top u\in C\rast\text{ where } C\rast := \{y \in\rr^n: \forall x\in C, \inner{x, y}\geq 0\}. \]
where $C\rast$ is known as the {\em dual cone} of $C$. 

\item The non-negativeness of $u\in\rr^m_+$ is independent of the primal cone $C$ of where $x$ is choosen. But dictated by the inequality direction $Mx\leq b$. Observe that if we instead have $Mx\geq b$, then we would take $Z = \{Ax : x\in R^n\} + R^m_-$, which then gives $y\leq0$.  
...
\end{array} 
\end{equation}
In particular, (P) has a solution if and only if (D) has a solution and, if (P) has a solution, then the maximum it attains corresponds to a solution to (D) that attains the same value as the minimum. 
