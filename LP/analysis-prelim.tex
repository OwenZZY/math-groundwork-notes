...
\begin{proposition}
Let $\bmy, \bmb_1, \bmb_2 \in\rr^m$. If $\bmb_1 \leq \bmb_2$ and $\bmy\geq 0$, then 
$$\inner{\bmy, \bmb_1}\leq \inner{\bmy, \bmb_2}.$$
\end{proposition}
\begin{proof}
Rewrite $\bmb_1 \leq \bmb_2$ as $\bmb_2 - \bmb_1 \geq 0$. As $\bmy\geq 0$ and every term we are dealing with is nonnegative, we have that $\inner{\bmy, \bmb_2 - \bmb_1} \geq 0$, which gives that $ \inner{\bmy, \bmb_2 - \bmb_1}  = \inner{\bmy, \bmb_2} - \inner{\bmy, \bmb_1} \geq 0 $ and thus $\inner{\bmy, \bmb_2} \geq \inner{\bmy, \bmb_1}$. 
\end{proof}
...
A set $C$, where $C\subset\rr^n$, is {\em convex} if for all $\bmx, \bmy\in C$ and $\alpha\in [0, 1]$, we have that $\alpha \bmx + (1-\alpha) \bmy\in C$. The closure and the interior of a convex set are also convex. A vector $\bmz$ has a linear combination of vectors $\bmx_1, \bmx_2, \dots, \bmx_m$ if there are $a_1, a_2, \dots, a_m\in\rr$ so that $z = a_1 \bmx_1 + a_2 \bmx_2 + \dots + a_m \bmx_m$. The linear combination is a {\em conic combination} if the coefficients are non-negative; that is, $a_i\geq 0$. The linear combination is a {\em convex combination} if it is a conic combination and $\sum_{i=1}^m a_i = 1$. 
...
The relative interval of a convex set is nonempty. 
\end{proposition}
\begin{proof} 
